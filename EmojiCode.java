...
// ANALIZADOR (ANALISIS SINTACTICO)
    ...
    static class CodigoIntermedio {
        ...
        String generarExpr(Expr e) {
            ...
            if (e instanceof ExprBinary b) {
                ...
                String opSimbolo = switch (b.op) {
                    ...
                    case "🔼"       -> ">";
                    case "🔽"       -> "<";
                    case "⏫"       -> "<="; // hasta/subiendo: contador <= limite
                    case "⏬"       -> ">="; // bajando: contador >= limite
                    case "🤝"       -> "&&";
                    case "🔀"       -> "||";
                    default         -> b.op;
                };
        ...
        // CONVIERTE CUÁDRUPLOS A CÓDIGO OBJETO / INTERMEDIO LEGIBLE
        String aCodigoObjeto() {
            if (cuadruplos.isEmpty()) return "No se generó código objeto.";

            StringBuilder sb = new StringBuilder();
            for (Cuadruplo c : cuadruplos) {
                switch (c.op) {
                    case "=":
                        sb.append("MOV ").append(c.result).append(", ").append(c.arg1).append("\n");
                        break;
                    case "+":
                        sb.append("ADD ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "-":
                        sb.append("SUB ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "*":
                        sb.append("MUL ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "/":
                        sb.append("DIV ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case ">":
                        sb.append("CMP_GT ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "<":
                        sb.append("CMP_LT ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case ">=":
                        sb.append("CMP_GE ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "<=":
                        sb.append("CMP_LE ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "==":
                        sb.append("CMP_EQ ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "!=":
                        sb.append("CMP_NE ").append(c.result).append(", ").append(c.arg1).append(", ").append(c.arg2).append("\n");
                        break;
                    case "if_false":
                        sb.append("JF ").append(c.arg1).append(", ").append(c.result).append("\n");
                        break;
                    case "goto":
                        sb.append("JMP ").append(c.result).append("\n");
                        break;
                    case "label":
                        sb.append(c.result).append(":\n");
                        break;
                    case "print":
                        sb.append("PRINT ").append(c.arg1).append("\n");
                        break;
                    case "read":
                        sb.append("READ ").append(c.result).append("\n");
                        break;
                    default:
                        sb.append(c.op).append(" ").append(c.arg1).append(", ").append(c.arg2).append(", ").append(c.result).append("\n");
                        break;
                }
            }
            return sb.toString();
    ...
    // =====================================================
    // GENERADOR DE ENSAMBLADOR PARA EMU8086 / DOS 16-BIT
    // Convierte cuádruplos a ASM compatible con EMU8086.
    // =====================================================
    static class GeneradorEMU8086 {

        private final Set<String> variables = new LinkedHashSet<>();
        private final Set<String> temporales = new LinkedHashSet<>();
        private final Map<String, String> variablesString = new LinkedHashMap<>();
        private final Set<String> variablesNumericas = new LinkedHashSet<>();
        private final List<String> dataExtra = new ArrayList<>();
        private int contadorMsg = 1;

        boolean esNumero(String valor) {
        ...
        String numeroEntero(String valor) {
            if (valor == null) return "0";
            if (valor.equalsIgnoreCase("true")) return "1";
            if (valor.equalsIgnoreCase("false")) return "0";
            try {
                return String.valueOf((int) Double.parseDouble(valor));
            } catch (NumberFormatException ex) {
                return "0";
            }
        }

        boolean esIdentificador(String valor) {
            if (valor == null) return false;
        ...
        boolean esString(String valor) {
            return valor != null && valor.length() >= 2
                    && valor.startsWith("\"") && valor.endsWith("\"");
        }

        String limpiarString(String valor) {
            return valor.substring(1, valor.length() - 1)
                    .replace("'", "")
                    .replace("$", "");
        }

        String asmNombre(String nombre) {
            if (nombre == null) return "";
            return nombre.replaceAll("[^a-zA-Z0-9_]", "_");
        }

        void recolectar(List<CodigoIntermedio.Cuadruplo> cuadruplos) {
            for (CodigoIntermedio.Cuadruplo c : cuadruplos) {
                // Detecta variables de texto por asignación: MOV mensaje, "Aprobado"
                if (c.op.equals("=") && esIdentificador(c.result) && esString(c.arg1)) {
                    variablesString.put(asmNombre(c.result), limpiarString(c.arg1));
                    continue;
                }

                revisarValor(c.arg1);
                revisarValor(c.arg2);
                revisarValor(c.result);
            }
        }

        void revisarValor(String valor) {
            if (valor == null || valor.equals("-")) return;
            if (esNumero(valor) || esString(valor)) return;
            if (valor.equalsIgnoreCase("true") || valor.equalsIgnoreCase("false")) return;
            if (valor.startsWith("L")) return;

            if (esTemporal(valor)) {
                temporales.add(asmNombre(valor));
            } else if (esIdentificador(valor)) {
                String nombre = asmNombre(valor);
                if (!variablesString.containsKey(nombre)) {
                    variables.add(nombre);
                    variablesNumericas.add(nombre);
                }
            }
        }

        String generar(List<CodigoIntermedio.Cuadruplo> cuadruplos) {
            variables.clear();
            temporales.clear();
            variablesString.clear();
            variablesNumericas.clear();
            dataExtra.clear();
            contadorMsg = 1;
            recolectar(cuadruplos);

            StringBuilder sb = new StringBuilder();

            // EMU8086 es sensible a BOM UTF-8, acentos y algunas directivas.
            // Generamos formato .COM simple: la primera linea SIEMPRE es ASCII puro.
            sb.append("org 100h\n");
            sb.append("jmp inicio\n\n");
            for (String v : variables) {
                if (!variablesString.containsKey(v)) {
                    sb.append("    ").append(v).append(" dw 0\n");
                }
            }
            for (Map.Entry<String, String> e : variablesString.entrySet()) {
                sb.append("    ").append(e.getKey()).append(" db '").append(e.getValue()).append("','$'\n");
            }
            for (String t : temporales) {
                sb.append("    ").append(t).append(" dw 0\n");
            }
            sb.append("    salto db 13,10,'$'\n");
            sb.append("    buffer db 6 dup('$')\n");
            sb.append("\n");
            ...
            sb.append(rutinaPrintNum());
            sb.append("\nend inicio\n");

            // Inserta mensajes string declarados durante la generación
            String texto = sb.toString();
            if (!dataExtra.isEmpty()) {
                StringBuilder extras = new StringBuilder();
                for (String d : dataExtra) {
                    extras.append(d).append("\n");
                }
                texto = texto.replace("    buffer db 6 dup('$')\n",
                        "    buffer db 6 dup('$')\n" + extras);
            }
        ...
        void generarCuadruplo(CodigoIntermedio.Cuadruplo c, StringBuilder sb) {
            switch (c.op) {
                case "=" -> {
                    if (esString(c.arg1)) {
                        // Variable de texto: ya fue declarada en .data como DB.
                        sb.append("    ; ").append(asmNombre(c.result)).append(" = texto\n\n");
                    } else {
                        cargarAX(c.arg1, sb);
                        sb.append("    mov ").append(asmNombre(c.result)).append(", ax\n\n");
                    }
                }

                case "+" -> {
                    cargarAX(c.arg1, sb);
                    sb.append("    add ax, ").append(operando(c.arg2)).append("\n");
                    sb.append("    mov ").append(asmNombre(c.result)).append(", ax\n\n");
                }

                case "-" -> {
                    cargarAX(c.arg1, sb);
                    sb.append("    sub ax, ").append(operando(c.arg2)).append("\n");
                    sb.append("    mov ").append(asmNombre(c.result)).append(", ax\n\n");
                }

                case "*" -> {
                    cargarAX(c.arg1, sb);
                    sb.append("    mov bx, ").append(operando(c.arg2)).append("\n");
                    sb.append("    imul bx\n");
                    sb.append("    mov ").append(asmNombre(c.result)).append(", ax\n\n");
                }
                ...
                case ">", "<", ">=", "<=", "==", "!=" -> {
                    generarComparacion(c, sb);
                }

                case "if_false" -> {
                    sb.append("    cmp ").append(asmNombre(c.arg1)).append(", 0\n");
                    sb.append("    je ").append(c.result).append("\n\n");
                }

                case "goto" -> sb.append("    jmp ").append(c.result).append("\n\n");

                case "label" -> sb.append(c.result).append(":\n");

                case "print" -> generarPrint(c.arg1, sb);

                case "read" -> {
                    sb.append("    ; READ no implementado en EMU8086\n");
                    sb.append("    ; Variable destino: ").append(c.result).append("\n\n");
                }

                default -> {
                    sb.append("    ; Operacion no soportada: ")
                            .append(c.op).append(" ")
                            .append(c.arg1).append(" ")
                            .append(c.arg2).append(" ")
                            .append(c.result).append("\n\n");
                }
            }
        }

        void generarComparacion(CodigoIntermedio.Cuadruplo c, StringBuilder sb) {
            String labelTrue = "TRUE_" + c.result;
            String labelEnd = "END_" + c.result;

            cargarAX(c.arg1, sb);
            sb.append("    cmp ax, ").append(operando(c.arg2)).append("\n");

            String salto = switch (c.op) {
                case ">" -> "jg";
                case "<" -> "jl";
                case ">=" -> "jge";
                case "<=" -> "jle";
            ...
            sb.append("    ").append(salto).append(" ").append(labelTrue).append("\n");
            sb.append("    mov ").append(asmNombre(c.result)).append(", 0\n");
            sb.append("    jmp ").append(labelEnd).append("\n");
            sb.append(labelTrue).append(":\n");
            sb.append("    mov ").append(asmNombre(c.result)).append(", 1\n");
            sb.append(labelEnd).append(":\n\n");
        }

        void generarPrint(String valor, StringBuilder sb) {
            if (esString(valor)) {
                String nombreMsg = "msg" + contadorMsg++;
                dataExtra.add("    " + nombreMsg + " db '" + limpiarString(valor) + "',13,10,'$'");
                sb.append("    lea dx, ").append(nombreMsg).append("\n");
            ...
            String nombre = asmNombre(valor);
            if (variablesString.containsKey(nombre)) {
                sb.append("    lea dx, ").append(nombre).append("\n");
                sb.append("    mov ah, 09h\n");
                sb.append("    int 21h\n");
                sb.append("    lea dx, salto\n");
                sb.append("    mov ah, 09h\n");
                sb.append("    int 21h\n\n");
                return;
            }
        ...
        void cargarAX(String valor, StringBuilder sb) {
            if (valor == null || valor.equals("-")) {
                sb.append("    mov ax, 0\n");
            } else if (valor.equalsIgnoreCase("true") || valor.equalsIgnoreCase("false") || esNumero(valor)) {
                sb.append("    mov ax, ").append(numeroEntero(valor)).append("\n");
            } else if (esString(valor)) {
                sb.append("    mov ax, 0\n");
            } else {
                sb.append("    mov ax, ").append(asmNombre(valor)).append("\n");
            }
        }

        String operando(String valor) {
            if (valor == null || valor.equals("-")) return "0";
            if (valor.equalsIgnoreCase("true") || valor.equalsIgnoreCase("false") || esNumero(valor)) {
                return numeroEntero(valor);
            }
            return asmNombre(valor);
        }

        String rutinaPrintNum() {
            return """
print_num proc
    ; imprime numero entero con signo en AX
    push ax
    push bx
    push cx
    push dx
...
""";
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            //MOSTRAR ASM PARA EMU8086
            JTextArea codigoASMArea = new JTextArea();
            codigoASMArea.setEditable(false);
            codigoASMArea.setFont(new Font("Courier New", Font.PLAIN, 13));
            ...
            tabs.add("ASM EMU8086", new JScrollPane(codigoASMArea));
            ...
            JButton guardarASM = new JButton("💾 Guardar ASM");
            ...
            // 💾 GUARDAR ASM EMU8086
            guardarASM.addActionListener(e -> {
                if (codigoASMArea.getText().trim().isEmpty()) {
                    JOptionPane.showMessageDialog(
                            f,
                            "❌ Primero ejecuta el programa para generar ASM.",
                            "ASM no disponible",
                ...
                JFileChooser chooser = new JFileChooser();
                chooser.setSelectedFile(new java.io.File("programa.asm"));
                chooser.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("Archivos ASM (.asm)", "asm"));
                int result = chooser.showSaveDialog(f);
                ...
                if (result == JFileChooser.APPROVE_OPTION) {
                    try {
                        ...
                        java.nio.file.Files.writeString(java.nio.file.Paths.get(ruta), codigoASMArea.getText());
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(
                                f,
                                "❌ Error al guardar el ASM",
                                "Error",
                                JOptionPane.ERROR_MESSAGE
                        );
                    }
            ...
            run.addActionListener(e -> {
                ...
                try {
                    ...
                    if(!reporter.hasErrors()){
                        ...
                        ci.generar(programa);
                        codigoIntermedioArea.setText(ci.aCodigoObjeto());
                        ...
                        // GENERAR ASM COMPATIBLE CON EMU8086
                        GeneradorEMU8086 genASM = new GeneradorEMU8086();
                        codigoASMArea.setText(genASM.generar(optimizados));

                        salida.append("COMPILACIÓN EXITOSA\n");
                        salida.append("ASM EMU8086 generado correctamente.\n");                       
                    }
